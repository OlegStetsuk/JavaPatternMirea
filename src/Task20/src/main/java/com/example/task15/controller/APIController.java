...
public class APIController {
    ...
    public ResponseEntity getDogPatient(@PathVariable("dogId") Long dogId) {
        try {
            return ResponseEntity.ok(dogServiceImpl.getPatientByDog(dogId));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Error: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity getDogsFiltered(@PathVariable("field") String field) {
        try {
            return ResponseEntity.ok(dogServiceImpl.getFilteredDogs(field));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Error: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity getPatientsFiltered(@PathVariable("field") String field) {
        try {
            return ResponseEntity.ok(patientServiceImpl.getFilteredPatients(field));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body("Error: " + e.getMessage());
        }
    }
